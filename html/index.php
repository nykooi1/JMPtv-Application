...
if ( isset($_GET["jmpforce"]) ) {
	if (strtolower($_GET["jmpforce"]) == "tv") {
		setcookie("Visyfy", "TV", time() + 3600*24*365*9, "/");
		header("Location: authorized.php");
		exit;
	}
} else if ( ! isset($_COOKIE["Visyfy"]) ) {
	header("Location: /help.php");
	exit;
}

if ( is_dir("accounts".$_SERVER["REQUEST_URI"]) ) {
	$vShow = vLabel($_SERVER["REQUEST_URI"]);
} else {
	header("Location: /help.php");
	exit;
}
...
<body>
<div id='vClickMessage'>Click to load first image</div>
...
function vLooper(){
	...
	if(vSlides.length == 0){
		vSlide = 0;
		$("#vClickMessage").css("visibility", "visible");
	...
	//if there is only one image, display the one image
		...
		vSlide = 0;
		$("#vClickMessage").css("visibility", "hidden");
		$("body").css("background-image", "url('" + vSlides[0] + "')");
	...
	//if there is more than one image, loop through them
		...
		$("#vClickMessage").css("visibility", "hidden");
		//change body to new image while invisible
		$("body").css("background-image", "url('" + vSlides[vNextSlide] + "')");
		//fade out cover
		setTimeout(function(){
			vSlide++;
...
vTimerHandle = setTimeout(function(){vLooper()}, 6000);
